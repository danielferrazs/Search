...
- Sites whose redirects carry a binary content type — `youtube.com` sends `application/binary` on its move to `www.youtube.com` — are followed to their destination instead of being downloaded as a file.
